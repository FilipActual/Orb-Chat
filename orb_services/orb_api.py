...
application = Flask(__name__, static_folder='/root/Orb-Chat/public/')
application.config['UPLOAD_FOLDER'] = '/root/Orb-Chat/public/media/'
...
def orb_login():
    ...
    with open('/root/Orb-Chat/public/pages/orb_login.html', 'r') as ui_code_source:
...
#ex
    ...
    if join_key in orb_access_keys:
        ...
        with open('/root/Orb-Chat/public/pages/orb_join.html', 'r') as ui_code_source:
...
def orb_feed():
    ...
    with open('/root/Orb-Chat/public/pages/orb_feed.html', 'r') as ui_code_source:
        ui_code = ui_code_source.read()
...
def send_posts():
    ...
    if user_key in orb_allowed_peers.keys():
        ...
        post_string = "<div class='profile-header'><img src=" + profile_image + " class='profile-image'></img><div class='name'><b>" + profile_name + "</b><div class='profile-bubble'><b class='handle'>@" + profile_handle + "</b></div></div></div><div class='post-body'>" + updated_local_records_source[local_record][1].replace('``', '<div class="small-spacer"></div>') + "<div class='small-spacer'></div><div id='image-area' class='image-area'>{}</div></div><div id='submit-button' class='reactor'><div id='like-button' metadata='" + local_record + "-like'><img id='" + local_record + "-like' class='" + like_class + "' src='" + like_button_src + "'></img></div><div metadata='" + local_record + "-like-count' class='" + like_count_class + "'>" + current_like_count + "</div><div id='comment-button' metadata='" + local_record + "-comment'><img  class='comment-button' src='data:image/png;base64,'></img></div><div metadata='" + local_record + "-count' class='comment-count'>8</div></div>"
...
def send_comments():
    ...
    if user_key in orb_allowed_peers.keys():
        ...
        with open('/root/Orb-Chat/public/feeds/orb_feed.json','r') as local_records_path:
            ...
            updated_local_records_source = json.load(local_records_path)
            # "commentid": [time created, text_content, file_actions, uuk]
        ...
        if comment_id == "-1": #start new comment request stream
        ...
        else: # next comment id has been supplied

            local_record = comment_id
            current_id = local_record
        ...
        comment_user_record = orb_users[uuk]

        profile_image = url_for('static', filename='media/' + comment_user_record[2] + '') # = profile image of screenname from comment, found in orb_users.json

        profile_name = comment_user_record[1]

        profile_handle = comment_user_record[0]
        ...
        # likes_path = '/root/Orb-Chat/public/activity/comment_likes/' + local_record + '.json'
        # if os.path.exists(likes_path):
        #   with open(likes_path, 'r') as likes_path_record:
        #       get length of dict
        #       dict has {uuk of user that liked it : ""}
        ...
        for filename in file_names:
            ...
            if filename != '':
                ...
                # file_url = exec(str(image_count) + "file_item = url_for('static', filename='public/media/" + filename + ")")
                comment_images = comment_images + "<img id='" + filename + "' src='" + url_for('static', filename='media/' + filename) + "' class='image-item' style='width: 100%; max-width: 1000px; float: left;'></img>"
                
        # comment_string = "<div id='comment-headers' class='global-east-content-container-loaded'><div class='profile-header'><img src=" + profile_image + " class='profile-image'></img><div class='name'><b>Justin Filip</b><div class='profile-bubble'><b class='handle'>@RealJustinFilip</b></div></div></div><div class='comment-body'>" + updated_local_records_source[local_record][1].replace('``', '<div class="small-spacer"></div>') + "<div id='image-area' class='image-area'>{}</div></div><div id='submit-button' class='reactor'><div metadata='" + local_record + "-count' class='" + like_count_class + "'><b>432</b></div><div id='like-button' metadata='" + local_record + "' class='" + like_class + "'><b>Like</b></div><div metadata='" + local_record + "-count' class='comment-count'><b>87</b></div><div id='comment-button' metadata='" + local_record + "' class='comment-button'><b>Comment</b></div><div metadata='" + local_record + "-count' class='share-count'><b>32</b></div><div id='share-button' metadata='" + local_record + "' class='share-button'><b>Share</b></div></div></div>"

        comment_string = "<div class='profile-header'><img src=" + profile_image + " class='profile-image'></img><div class='name'><b>" + profile_name + "</b><div class='profile-bubble'><b class='handle'>@" + profile_handle + "</b></div></div></div><div class='comment-body'>" + updated_local_records_source[local_record][1].replace('``', '<div class="small-spacer"></div>') + "<div class='small-spacer'></div><div id='image-area' class='image-area'>{}</div></div><div id='submit-button' class='reactor'><div id='like-button' metadata='" + local_record + "-like'><img id='" + local_record + "-like' class='" + like_class + "' src='" + like_button_src + "'></img></div><div metadata='" + local_record + "-like-count' class='" + like_count_class + "'>" + current_like_count + "</div><div id='comment-button' metadata='" + local_record + "-comment'><img  class='comment-button' src='data:image/png;base64,'></img></div><div metadata='" + local_record + "-count' class='comment-count'>8</div></div>"

        comments += comment_string.format(comment_images)

        return {'content':comments, 'cid':current_id, 'nid':next_id}
...
def orb_post():
    ...
    user_key = request.args['key']
    selected_post_id = request.args['id']
    ...
    if user_key in orb_allowed_peers.keys():
        ...
        # comment that contains media 
        def media_comment(file_actions):
            ...
            def write_record(text_content, file_actions):
                ...
                comment_string = text_content + file_actions
                comment_id = hmac.new(b'secret', comment_string.encode('utf-8'), hashlib.sha256).hexdigest()

                if os.path.exists(comments_path):

                    with open(comments_path,'r') as local_records_path:
                                    
                        updated_local_records_source = json.load(local_records_path)

                    if comment_id not in updated_local_records_source:

                        timestamp = float(time.time())
                        updated_local_records = {comment_id:[timestamp, text_content, file_actions, uuk]}
                        updated_local_records.update(updated_local_records_source)

                        with open(comments_path, 'w') as local_records_path:
        ...
        def text_comment():
            ...
            def write_record(text_content):
                ...
                comment_id = hmac.new(b'secret', text_content.encode('utf-8'), hashlib.sha256).hexdigest()
                ...
                if text_content == '':
                    ...
                    if os.path.exists(comments_path):
                        ...
                        if comment_id not in updated_local_records_source:
                            ...
                            timestamp = float(time.time())
                            updated_local_records = {comment_id:[timestamp, text_content, '']}
                            updated_local_records.update(updated_local_records_source)
